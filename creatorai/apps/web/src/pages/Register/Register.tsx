...
export default function Register() {
  ...
  const clearFieldError = (key: 'name' | 'username' | 'email' | 'password') =>
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) fe.name = 'Enter your name.';
    if (!username.trim()) fe.username = 'Choose a username.';
    else if (!/^[a-zA-Z0-9_]{3,20}$/.test(username.trim()))
      fe.username = 'Use 3–20 letters, numbers, or underscores.';
    if (!email.trim()) fe.email = 'Enter your email.';
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim()))
      fe.email = 'Enter a valid email address.';
    if (!password) fe.password = 'Enter a password.';
    else if (password.length < 8) fe.password = 'Password must be at least 8 characters.';
    setFieldErrors(fe);
    if (fe.name || fe.username || fe.email || fe.password) return;
    ...
    <div className={styles.root}>
      ...
      <div className={styles.card}>
        <div className={styles.head}>
          ...
          <p className={styles.subtitle}>Start free — 20 credits included</p>
        </div>
            ...
            <div>
              ...
              {fieldErrors.name && (
                <p className={styles.fieldError}>
                  <AlertCircle className={styles.fieldErrorIcon} /> {fieldErrors.name}
                </p>
              )}
            </div>
            <div>
              ...
              {fieldErrors.username && (
                <p className={styles.fieldError}>
                  <AlertCircle className={styles.fieldErrorIcon} /> {fieldErrors.username}
                </p>
              )}
            </div>
            <div>
              ...
              {fieldErrors.email && (
                <p className={styles.fieldError}>
                  <AlertCircle className={styles.fieldErrorIcon} /> {fieldErrors.email}
                </p>
              )}
            </div>
            <div>
